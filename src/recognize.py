...
Haar (multi-face)
-> MediaPipe FaceLandmarker 5pt (per-face ROI)
...
Notes:
- We run MediaPipe FaceLandmarker on EACH Haar face ROI.
- This keeps the same multi-face logic as before.
...
class HaarFaceMesh5pt:
    ...
    def __init__(
        ...
        if not model_path.exists():
            ...
            raise RuntimeError(
                ...
                f"{model_path}\n"
                f"Expected file: models/face_landmarker.task"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Your installed MediaPipe does not expose "
                "the expected Tasks API. "
                f"MediaPipe version: "
        ...
        # MediaPipe FaceLandmarker landmark
        # indices corresponding to the same
        # points used by the previous FaceMesh code.
    ...
    def detect(
        ...
        for (x, y, w, h) in faces:
            ...
            # Expand ROI for FaceLandmarker stability
            mx = 0.25 * w
            my = 0.35 * h
